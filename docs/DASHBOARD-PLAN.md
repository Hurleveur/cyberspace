...
**Default view:** Centered on your location (see MAP_CENTER in .env), zoom level showing your region.
...
[
  ...
  {
    "id": "event-local-security-meetup",
    "type": "event",
    "priority": "high",
    "title": "Local Security Meetup March",
    "summary": "Monthly local security community meetup.",
    "source_url": "https://...",
    "location_label": "Your City, Your Country",
    "date": "2026-03-15"
  }
]
```
...
from the feed source (e.g. CERT-EU → Your City, CISA → Washington DC).
...
# Add your national CERT feed here:
# - https://your-national-cert.example.com/rss.xml [MEDIUM]
...
```
┌──────────────────────────────────┐
│ Local Security Meetup March         ★★★★│
│ ─────────────────────────────────│
│ When: March 15, 18:30            │
│ Where: City Convention Centre, Your City │
│ Cost: Free                       │
...
2. Frontend sends POST to `/api/feedback` with:
   `[2026-03-01] EVENT ACCEPTED: "Local Security Meetup March" (March 15, Your City)`
...
2. Frontend sends POST to `/api/feedback` with:
   `[2026-03-01] EVENT SKIPPED: "Local Security Meetup March" — not interested`
...
2. **RSS feed items:** Harder. The server uses heuristics:
   - Feed source location (CISA → Washington DC, CERT-EU → Your City, or your national CERT location)
...
- [ ] Centered on your location (see MAP_CENTER in .env), zoom level showing your region
