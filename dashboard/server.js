...
// Helper: parse briefing markdown for threat level + metric counts
function parseBriefingStats(markdown) {
  const emojiScore = { '🟢': 1, '🟡': 2, '🟠': 3, '🔴': 4 };

  let streak = 0;
  let threatEmoji = null;
  let threatLevel = null;

  // Try canonical format: Briefing #N · emoji LEVEL
  const streakMatch = markdown.match(/Briefing #(\d+)\s*·\s*(🟢|🟡|🟠|🔴)\s*(\w+)/u);
  if (streakMatch) {
    streak = parseInt(streakMatch[1], 10);
    threatEmoji = streakMatch[2];
    threatLevel = streakMatch[3].toUpperCase();
  } else {
    // Fallback: "Day N of daily briefings" footer
    const dayMatch = markdown.match(/[Dd]ay (\d+) of/);
    if (dayMatch) streak = parseInt(dayMatch[1], 10);

    // Fallback threat level: **Overall threat level:** emoji LEVEL
    const tMatch = markdown.match(/\*\*Overall threat level:\*\*\s*(🟢|🟡|🟠|🔴)\s*(\w+)/u)
                || markdown.match(/(🟢|🟡|🟠|🔴)\s*(LOW|GUARDED|ELEVATED|HIGH|SEVERE|CRITICAL)/iu);
    if (tMatch) {
      threatEmoji = tMatch[1];
      threatLevel = tMatch[2].toUpperCase();
    }
  }

  if (!threatEmoji || !threatLevel) return null;

  const threatScore = emojiScore[threatEmoji] || 0;

  // Parse metric table rows — extract first integer from each value cell
  const getMetric = (patterns) => {
    for (const pat of patterns) {
      const m = markdown.match(new RegExp(`\\|[^|]*${pat}[^|]*\\|([^|]+)\\|`, 'i'));
      if (m) return parseInt(m[1].replace(/[^\d]/g, '').slice(0, 4), 10) || 0;
    }
    return 0;
  };
  ...
  return {
    ...
    threatLevel,
    threatScore,
    cves:        getMetric(['CVE', 'notable']),
    exploits:    getMetric(['exploit', 'active']),
    breaches:    getMetric(['breach']),
    ransomware:  getMetric(['ransomware']),
  };
}

// GET /api/stats — threat level + metric history across all briefings
app.get('/api/stats', (req, res) => {
  ...
  for (const date of dates) {
    ...
    if (r.error) continue;
    const parsed = parseBriefingStats(r.content);
    if (parsed) stats.push({ date, ...parsed });
  }
